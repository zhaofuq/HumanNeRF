...
class Perceptual_loss(torch.nn.Module):
    ...
    def forward(self, x, target):
        # print(x.t, target.shape)
        x = x.float()
        target = target.float()
        x_feature = self.model(x[:, 0:3, :, :])
        target_feature = self.model(target[:, 0:3, :, :])

        x_feature_alpha = self.model(x[:, 3:4, :, :].repeat([1, 3, 1, 1]))
        target_feature_alpha = self.model(target[:, 3:4, :, :].repeat([1, 3, 1, 1]))

        xgrad_x = gradient_1order(x[:, 0:3, :, :])
        xgrad_target = gradient_1order(target[:, 0:3, :, :])

        feature_loss = self.loss(x_feature.relu1, target_feature.relu1) + self.loss(x_feature.relu2,
                                                                                    target_feature.relu2)
        feature_loss = feature_loss + self.loss(x_feature_alpha.relu1, target_feature_alpha.relu1) + self.loss(
            x_feature_alpha.relu2, target_feature_alpha.relu2)
        feature_loss = feature_loss / 4
        return feature_loss, self.loss(x, target), self.loss(xgrad_x, xgrad_target)
