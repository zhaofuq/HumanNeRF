...
def gen_weight(sigma, delta, act_fn=F.relu):
    ...
    """
    alpha = 1.-torch.exp(-act_fn(sigma.squeeze(-1))*delta) # (N,1)
    weight = 1.-alpha+1e-10 #(N,1)
...
class VolumeRenderer(nn.Module):
    ...
    def forward(self, depth, rgb, sigma, rays_norm, noise=0):
        """
