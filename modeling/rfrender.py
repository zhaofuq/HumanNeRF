...
# from .spacenet_deep import SpaceNet
...
class RFRender(nn.Module):
    ...
    def __init__(self, coarse_ray_sample, fine_ray_sample, boarder_weight, sample_method = 'DEPTH', same_space_net = False, TriKernel_include_input = True, cfg=None):
        ...
        if self.use_deform:
            ...
            self.deformnet = DeltaNet(feat_dim = cfg.MODEL.FEATURE_DIM * 6, include_input = TriKernel_include_input)
        #----------------------------------------------------------------DEFORMATION-----------------------------------------------------------------#


        #----------------------------------------------------------------NeRF Model-----------------------------------------------------------------#
        self.spacenet = SpaceNet(include_input = TriKernel_include_input, use_dir = self.use_dir, feature_dim = cfg.MODEL.FEATURE_DIM * 6)
        if same_space_net:
            self.spacenet_fine = self.spacenet
        else:
            self.spacenet_fine = SpaceNet(include_input = TriKernel_include_input, use_dir = self.use_dir, feature_dim = cfg.MODEL.FEATURE_DIM * 6)
    ...
    def forward(self, rays, bboxes, feature_maps,  Rts = None, global_ts = None, skeletons = None, vertices = None, scene = None, only_coarse = False, sample_depth = None , near_far=None, scale = None):
        ...
        if rays_t.size(0) > 1:
            ...
            if self.use_deform:
                ...
                #extract features
                features = self.feature_extractor(feature_maps, msampled_rays_coarse_xyz, scene)

                #non-rigid transformation
                offset = self.non_rigid_warper(skeletons, msampled_rays_coarse_xyz, features)
            ...
            mrgbs, mdensity  = self.spacenet(msampled_rays_coarse_xyz, ray_dirs , features, self.maxs, self.mins)
            rgbs[w_mask] = mrgbs
            ...
            if not only_coarse:
                ...
                if self.use_deform:
                    ...
                    #extract features
                    features_fine = self.feature_extractor(feature_maps, msamples_fine_xyz, scene)
                    
                    #non-rigid transformation
                    offset_fine  = self.non_rigid_warper(skeletons, msamples_fine_xyz, features_fine)
                ...
                mrgbs_fine, mdensity_fine = self.spacenet_fine(msamples_fine_xyz, ray_dirs_fine, features_fine, self.maxs, self.mins)
