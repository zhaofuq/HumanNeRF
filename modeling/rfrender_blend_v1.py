...
class RFRender(nn.Module):
    ...
    def __init__(self, coarse_ray_sample, fine_ray_sample, boarder_weight, sample_method = 'DEPTH', same_space_net = False, TriKernel_include_input = True, cfg=None):
        ...
        if self.use_deform:
            ...
            self.deformnet = DeltaNet(feat_dim = cfg.MODEL.FEATURE_DIM, include_input = TriKernel_include_input)
            self.blendnet = BlendNet(feat_dim = cfg.MODEL.FEATURE_DIM * 6, include_input = TriKernel_include_input)
        #----------------------------------------------------------------DEFORMATION-----------------------------------------------------------------#


        #----------------------------------------------------------------NeRF Model-----------------------------------------------------------------#
        self.spacenet = SpaceNet(include_input = TriKernel_include_input, use_dir = self.use_dir, feature_dim = cfg.MODEL.FEATURE_DIM)
        if same_space_net:
            self.spacenet_fine = self.spacenet
        else:
            self.spacenet_fine = SpaceNet(include_input = TriKernel_include_input, use_dir = self.use_dir, feature_dim = cfg.MODEL.FEATURE_DIM)
    ...
    def forward(self, rays, bboxes, feature_maps,  Rts = None, global_ts = None, skeletons = None, vertices = None, scene = None, only_coarse = False, sample_depth = None , near_far=None, scale = None):
        ...
        if rays_t.size(0) > 1:
            ...
            if self.use_deform:
                ...
                # blend features
                weights = self.blendnet(angle, ray_dirs, features.reshape(-1, features.size(1)*features.size(2)))
                # if weights.shape[0] > 0:
                #     print(f"weights:{weights[0]}")

                blend_features = torch.sum(weights.unsqueeze(-1) * features, dim = 1)
