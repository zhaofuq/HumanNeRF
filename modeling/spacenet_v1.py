...
class SpaceNet(nn.Module):
    ...
    def __init__(self, c_pos=3, include_input = True,use_dir = True, feature_dim = 128):
        ...
        self.stage1 = nn.Sequential(
                    nn.Linear(self.pos_dim + self.feat_dim, backbone_dim),
        ...
        self.stage2 = nn.Sequential(
                    nn.Linear(backbone_dim + self.pos_dim + self.feat_dim, backbone_dim),
                    nn.ReLU(inplace=True),
                    nn.Linear(backbone_dim,backbone_dim),
                    nn.ReLU(inplace=True),
                    nn.Linear(backbone_dim,backbone_dim),
        ...
        self.rgb_net = nn.Sequential(
                    nn.ReLU(inplace=True),
                    nn.Linear(backbone_dim + self.dir_dim + self.feat_dim, backbone_dim),
                    nn.ReLU(inplace=True),
    ...
    '''
    def forward(self, pos, rays, feats = None, maxs=None, mins=None):
        ...
        #beg = time.time()
        x = self.stage1(torch.cat([pos, feats],dim =1))
        x = self.stage2(torch.cat([x, pos, feats],dim =1))

        density = self.density_net(x)

        if rays is not None and self.use_dir:
            if self.feat_dim != 0:
                rgbs = self.rgb_net(torch.cat([x,dirs,feats],dim =1))
            else:
